...
std::vector<ConfigItem<bool>> CFG::getBoolItems()
{
    return {
        ...
        // ### TW:  add sensor switching (on/off)
        {"image.alt_sensor", image.alt_sensor, false, validateBool},
        // ### TW:  exposure mode (auto/manual)
        {"image.core_expr_mode", image.core_expr_mode, true, validateBool},         
        // ### TW:  AGain manual enable   
        {"image.again_mode", image.again_mode, false, validateBool}, 
#if 0
        // ### TW:  add webui gpio support
        {"gpio.ir850", gpio.ir850, false, validateBool}, 
        {"gpio.ir940", gpio.ir940, false, validateBool}, 
        {"gpio.white", gpio.white, false, validateBool}, 
        {"gpio.ircut", gpio.ircut, false, validateBool}, 
        {"gpio.sensor_switch", gpio.sensor_switch, false, validateBool}, 
        {"gpio.daynight", gpio.daynight, false, validateBool}, 
#endif       
        ...
        {"stream0.direct_mode", stream0.direct_mode, false, validateBool},
        ...
        {"stream1.direct_mode", stream1.direct_mode, false, validateBool},
...
std::vector<ConfigItem<int>> CFG::getIntItems()
{
    return {
        ...
        {"image.running_mode", image.running_mode, 0, validateInt2},  // allow feedback
        ...
        // ### TW:  rate for sensor sw: 10 ms - 30000 ms
        {"image.alt_speed", image.alt_speed, 0, [](const int &v) { return v >= 10 && v <= 30000; }}, 
        // ### TW:  exposure mode
        {"image.core_expr_time", image.core_expr_time, 500, [](const int &v) { return v >= 0 && v <= 1000; }},
        // ### TW:  AGain manual value
        {"image.again_gain", image.again_gain, 15000, [](const int &v) { return v >= 0 && v <= 1000000; }},
#if 1
        // ### TW:  add webui gpio support, use int to allow status feedback.  0: off, 1: on, 2: allow current state feedback
        {"gpio.sensor_switch", gpio.sensor_switch, 1, validateInt2},  
...
#if 0  // XXXXXX FLASH killer? *** XXXXXXX write to file xxxxxx
    // Save list of hits to config - only one dimension, not nested...
    std::string hit_list_value = "";
    std::string hit_list_Path = "rois_hit";
    for (int i = 0; i < motion.roi_count; i++)
    {
        // Create array string: [hit0, hit1,.....]
        hit_list_value = hit_list_Value + std::to_string(i) + ",";
    }
    hit_list_value = hit_list_value.substr(0,hit_list_value.size()-1);  // remove last ","
    setNestedValue(jsonConfig, hit_list_Path, hit_list_value);
#endif 
...
void CFG::load()
{
    ...
    // ### TW:  load current ROIs from cfg file
