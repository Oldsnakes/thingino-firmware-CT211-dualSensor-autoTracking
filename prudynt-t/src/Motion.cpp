...
//            if (tracking_enabled && isMotorActive && duration_cast<milliseconds>(currentTime - motorMoveEndTime).count() < cfg->motion.move_time)
...
//                    debounce++;
            ...
            if (motionDetected) {
                ...
                // cfg->motion.rois_hit_list = msg_json;
                debounce++;
                int i, j;
                ...
                if (tracking_enabled)
                {
                    ...
                    LOG_DEBUG("CENTER:" << CENTER << " = " << box_weight[CENTER] << " Total = " << hit_total << " roi count = " << cfg->motion.roi_count);
            ...
            if (!motionDetected)
            {
                ...
                if (moving && duration >= cfg->motion.min_time && duration >= cfg->motion.post_time)
                {
                    ...
                    // ### TW - end of light event after post_time
...
//        move_param.roiRectCnt = cfg->motion.roi_count;
    ...
    if (cfg->motion.mapMode && tracking_enable) { // on PTZ
