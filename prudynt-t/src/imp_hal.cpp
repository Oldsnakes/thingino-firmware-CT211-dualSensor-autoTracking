...
{
    char cmd = MOTOR_GET_POSITION;
    int value = 0;
//    std::string s(1, cmd); 
//    LOG_DEBUG("motor command: <" << s << "> value: " << value);
//    Motor::motor_action(cmd,value);
    ...
    Motor::motor_action(cmd,value);
    cmd = MOTOR_DIR;
    value = MOTOR_REL_POS; //103;
    Motor::motor_action(cmd,value);
    sleep(1);
    cmd = MOTOR_JSON_STATUS;
    value = 0;
    Motor::motor_action(cmd,value);
    return 0;
}
...
#if 0  // **** for test sync timing
int set_core_expr_time(unsigned int val)
{
    global_time_delay = val;
    return 0;
}
#endif // ####
...
{
    ctrls_hal::setGPIObyName("gpio_ir850", enable);
    return 0;
}

int set_ir940(bool enable)
{
    ctrls_hal::setGPIObyName("gpio_ir940", enable);
    return 0;
}
int set_white(bool enable)
{
    ctrls_hal::setGPIObyName("gpio_white", enable);
    return 0;
}
int set_sensor_switch(bool enable)
{
#ifdef IMP_DEBUG
    LOG_DEBUG("set sensor switch" << enable);
#endif
    ctrls_hal::setGPIObyName("gpio_sensor_switch", enable);
    return 0;
}
#endif
// ######
