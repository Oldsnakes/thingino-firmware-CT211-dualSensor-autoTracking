...
int32_t time_pading = 0;


// ### TW : intermeidate buffer for jpeg snapshot stream
//std::shared_ptr<jpeg_stream> jpeg_alt_stream[NUM_JPEG_CHANNELS] = {nullptr};
//char * alt_jpeg_stream[NUM_JPEG_CHANNELS];
// ####
...
void JPEGWorker::run()
{
    ...
    int64_t time_diff = 0;
    // int32_t time_pading = 0;
    int32_t time_frame = 100 * 1000;  // 10 fps
    ...
    global_jpeg[jpgChn+2]->stream->stats.ts = global_jpeg[jpgChn]->stream->stats.ts;
    global_jpeg[jpgChn]->stream->stats.ts.tv_sec -= 10;
    global_jpeg[jpgChn+2]->stream->stats.ts.tv_sec -= 10;

    while (global_jpeg[jpgChn]->running)
    {
        /*
        * if jpeg_idle_fps = 0, the thread is put into sleep until a client is connected.
        * if jpeg_idle_fps > 0, we try to reach a frame rate of stream.jpeg_idle_fps. even if no client is connected.
        ...
        if (request_or_overrun || targetFps)
        {
            ...
            if (targetFps && diff_last_image >= ((1000 / targetFps) - targetFps / 10))
            {
                ...
                if (global_jpeg[jpgChn]->reconfig.load())
                {
                    ...
                    {
            #ifdef JPEG_DEBUG
                        LOG_DEBUG("* jpgChn = " << jpgChn << " new w = " << new_w << ", h = " << new_h << ", fps = " << new_fps);
            #endif
                        ...
                        }
                        {
                            ...
                            // ### TW
                                if (global_MipiMode == 3) {  
                                    time_diff = stream.pack[i].timestamp - last_sw_time + time_pading;
                                    time_frame = 1000000 / global_jpeg[jpgChn]->stream->fps;  // us
                                    pad_time =  time_frame / 10;  // us
                                    /* process mipi switching based on time lapes */
            #ifdef JPEG_DEBUG
                            LOG_DEBUG("<#" << jpgChn << "#> pack_time = " << stream.pack[i].timestamp << " last_sw_time = " << last_sw_time 
                              << " time_diff = " << time_diff );
                                // << " len = " << data_len << " frameEnd: " << stream.pack[i].frameEnd );
            #endif
                                    if  (current_sensor != global_Mipi_sw_state) {  // state changed
                                        ...
                                        LOG_DEBUG("<#" << jpgChn << "#> pack_time = " << stream.pack[i].timestamp << " last_sw_time = " << last_sw_time 
                                                << " time_diff = " << time_diff );
            #endif
                                        // ### calculate pading only for ch0 for both encoder channels
                                        if (time_diff > 0) { // the pack was encoded after MIPI switched to new sensor - Switch.
                                            current_sensor = global_Mipi_sw_state;   
            #ifdef JPEG_DEBUG
                                            LOG_DEBUG("<#" << jpgChn << "#> jpeg sw_state ******* " << current_sensor << " *** pad = " << time_pading
                                                << " frame time = " << time_frame);
            #endif
                                            if ((jpgChn == 0) && (time_diff > time_frame/2)) {  // it take too long to switch, reduce pad.
                                                time_pading -= pad_time;  // us
            #ifdef JPEG_DEBUG
                                                LOG_DEBUG("<#" << jpgChn << "#> --- jpeg skew =  " << time_pading << " ---" );
            #endif
                                            }
                                    ...
                                    alt_chn = current_sensor * 2 + jpgChn;  
                            #if 0
                                    buf = global_jpeg[alt_chn]->snapshot_buf;
                                    buf.resize(total_size);
                                    dst = buf.data();
                            #endif
                                    unsigned char *dst = nullptr;
                                    if (alt_chn == 0) {
                                        auto& buf = global_jpeg[0]->snapshot_buf;
                                        buf.resize(total_size);
                                        dst = buf.data();
                                    } else if (alt_chn == 1) {
                                        auto& buf = global_jpeg[1]->snapshot_buf;
                                        buf.resize(total_size);
                                        dst = buf.data();
                                    } else if (alt_chn == 2) {
                                        auto& buf = global_jpeg[2]->snapshot_buf;
                                        buf.resize(total_size);
                                        dst = buf.data();
                                    } else if (alt_chn == 3) {
                                        auto& buf = global_jpeg[3]->snapshot_buf;
                                        buf.resize(total_size);
                                        dst = buf.data();
                                    } else {
                                        auto& buf = global_jpeg[0]->snapshot_buf;
            ...
            #ifdef JPEG_DEBUG
                                    //LOG_DEBUG(" * JPEG * sensor: " << current_sensor << " <(" << jpgChn << ")> -> : " << alt_chn
                                    //           << " len: " << data_len << " time: : " << stream.pack[i].timestamp);
//                                    sprintf(msg," - buf %p: 0x%08x <- snapshot_buf %p 0x%08x ", buf, buf, 
//                                        global_jpeg[alt_chn]->snapshot_buf, global_jpeg[alt_chn]->snapshot_buf);
//                                    LOG_DEBUG(msg);
            #endif
...
//                    if (jpgChn == global_Mipi_sw_state) {
                    if (jpgChn <= 1) {
...
void *JPEGWorker::thread_entry(void *arg)
{
    ...
    */
    ...
    // ### TW
    /*   1-FEED-2 */
    /*  jpegChn 0/2:  streamChn = 0, main, sensor 0/1 */
    /*  jpegChn 1/3:  streamChn = 1, sub, sensor 0/1 */
    global_jpeg[jpgChn]->streamChn = global_jpeg[jpgChn]->stream->jpeg_channel;   // 0/1
    global_jpeg[jpgChn+2]->streamChn = global_jpeg[jpgChn]->stream->jpeg_channel; // 2/3
    ...
    if (global_jpeg[jpgChn]->streamChn == 0)
    {
        ...
        global_jpeg[jpgChn+2]->stream->width = cfg->stream0.width;
        global_jpeg[jpgChn+2]->stream->height = cfg->stream0.height;
    }
    else
    {
        global_jpeg[jpgChn]->stream->width = cfg->stream1.width;
        global_jpeg[jpgChn]->stream->height = cfg->stream1.height;
        global_jpeg[jpgChn+2]->stream->width = cfg->stream1.width;
        global_jpeg[jpgChn+2]->stream->height = cfg->stream1.height;
    }

    LOG_DEBUG("<" << jpgChn << "> alt_chn: " << jpgChn+2 << " - streamChn: " << global_jpeg[jpgChn]->streamChn << " W: " 
        << global_jpeg[jpgChn]->stream->width << " H: " << global_jpeg[jpgChn]->stream->height);
    global_jpeg[jpgChn]->imp_encoder = IMPEncoder::createNew(global_jpeg[jpgChn]->stream,
                                                            sh->encChn,  // channel
                                                            global_jpeg[jpgChn]->streamChn,  // group
                                                            "stream2");
    ...
    global_jpeg[jpgChn+2]->imp_encoder = global_jpeg[jpgChn]->imp_encoder;  
    ...
    global_jpeg[jpgChn+2]->active = true;
    global_jpeg[jpgChn+2]->running = true;
