 ...
 *  ### TW:   modified to dup i2c bus activity for double sensor devices.
 *
...
/* ### TW: */
#define IOMEM_OFFSET 	0x00001000
/*#define I2C_SWAP	   // swapped sensor i2c bus
#ifdef I2C_SWAP
	int32_t i2c_iomem_offset = -0x00001000;  // used in i2c_readl/i2c_writel
	int I2C_CTRL_CLK_OFFSET = -1;	// not used, reference only
	int I2C_CTRL_IRQ_OFFSET = 1;	// not used, reference only
#else
*/
	int32_t i2c_iomem_offset = 0x00001000;  // used in i2c_readl/i2c_writel
	int I2C_CTRL_CLK_OFFSET = 1;	// not used, reference only
		...
		unsigned short offset)
{
	...
	value = readl(i2c->iomem + offset);
#ifdef I2C_DR_DEBUG
//	printk("%s:<-i2c: R-name = %s(%d), iomem = 0x%08x, ret = 0x%x", 
	printk("%s: <- R-%s(%d), (0x%08x) = 0x%x", 
		__func__, i2c->adap.name, i2c->adap.nr, i2c->iomem + offset, value);
#endif
...
/* #### TW:  dup ctrl  */
	...
	uint address;
	if (i2c->adap.nr == 0) {
		if (((uint) i2c->iomem & 0x00001000)) {
			address = (uint) i2c->iomem + offset - IOMEM_OFFSET;
			ret = readl(i2c->iomem + offset - IOMEM_OFFSET);
		} else {
			address = (uint) i2c->iomem + offset + IOMEM_OFFSET;  // swapped i2c bus
			ret = readl(i2c->iomem + offset + IOMEM_OFFSET);
		}

#ifdef I2C_DR_DEBUG
//		printk(" == dup-R: iomem = 0x%08x, ret = 0x%x", address, ret);
		printk(" == dup-R: (0x%08x) = 0x%x", address, ret);
#endif
	}
#endif
#ifdef I2C_DR_DEBUG
...
#ifdef I2C_DR_DEBUG
//	printk("%s:->i2c: W-name = %s (%d), iomem = 0x%08x, value = 0x%x ", 
	printk("%s:-> W-%s(%d), (0x%08x) = 0x%x ", 
		__func__, i2c->adap.name, i2c->adap.nr, i2c->iomem + offset, value);
#endif
...
/* #### TW:  dup ctrl  */
	...
	if (i2c->adap.nr == 0) {
		if (((uint) i2c->iomem & 0x00001000)) {
			address = (uint) i2c->iomem + offset - IOMEM_OFFSET;
			writel(value, i2c->iomem + offset - IOMEM_OFFSET);
		} else {
			address = (uint) i2c->iomem + offset + IOMEM_OFFSET;  // swapped i2c bus
			writel(value, i2c->iomem + offset + IOMEM_OFFSET);
		}
...
#ifdef I2C_DR_DEBUG
//  	printk(" == dup-W: iomem = 0x%08x", address);
  	printk(" == dup-W: (0x%08x) = 0x%x", address, value);
#endif
	}
#endif
#ifdef I2C_DR_DEBUG
...
/* #### TW:  need dispatch IRQ for dup setup ???  */
...
/* ### TW:  */
#ifdef I2C_DR_DEBUG
	printk("%s:  i2c:  name = %s (%d), iomem = 0x%08x, irq = %d, len = %d\n", __func__, i2c->adap.name, i2c->adap.nr, i2c->iomem, i2c->irq, len);
#endif
#ifdef I2C_DEBUG
...
/* ### TW:  */
#ifdef I2C_DR_DEBUG
	printk("%s: i2c:  name = %s (%d), iomem = 0x%08x, irq = %d \n", __func__, i2c->adap.name, i2c->adap.nr, i2c->iomem, i2c->irq);
#endif
 
	ret = request_irq(i2c->irq, i2c_jz_irq, IRQF_DISABLED,
