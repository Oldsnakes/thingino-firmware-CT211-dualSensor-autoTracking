...
/* ### TW:  alternate channels - 4/5 */
#define ALT_STREAM_SUPPORT
...
struct _motor {
    ...
    int     y;
    int     direction;
    int     move_mode;  // absolue/relative/home
    int     state;  // busy/active/disable
    int     home_x;
    int     home_y;
    int     max_x;
    int     max_y;
...
class CFG {
    ...
    private:
        ...
        std::vector<ConfigItem<const char *>> ROIsItems{};
        std::vector<ConfigItem<const char *>> getROIsItems();
...
extern std::atomic<int> global_MipiMode;
