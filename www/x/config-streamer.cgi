...
<div class="row g-2">
<div class="col-3"><% field_switch "image_hflip" "Flip image horizontally" %></div>
<div class="col-3"><% field_switch "image_vflip" "Flip image vertically" %></div>
</div>
<div class="row g-2">
...
<div class="col-3"><% field_range "image_core_expr_time" "Time - msec ( 0 - 1000 )" "0,1000,1" %></div>
<div class="col-1"><% field_space "space1" "" %></div>
<div class="col-2"><% field_switch "image_again_mode" "Manual Gain" %></div>
<div class="col-3"><% field_range "image_again_gain" "     Analog Gain (0-100,000)" "0,100000,100" %></div>
</div>
...
// gpio
const gpio_params = [ 'sensor_switch' ];
	...
	// Image
	{
		...
		if (data) {
                	if (msg.image.running_mode <= 1) 
                        	$('#image_running_mode').checked = (msg.image.running_mode == 1);
                	if (msg.image.alt_sensor == true ) {
				$(`#preview1`).style.display = 'block'; 
				$(`#preview1`).style.visibility = 'visible'; 
				if ($(`#preview_source_0`).checked) {
					$('#preview1').src='/x/ch2.mjpg';
				//	$('#preview_source_0').addEventListener('load', () => 
				// { 
	 			//	});
				// }
				} else {
					$('#preview1').src='/x/ch3.mjpg';
				//	$('#preview_source_1').addEventListener('load', () => 
				//	{ 
	 			//	});
				}
			}
...
gpio_params.forEach((x) => {
	...
	el.addEventListener('dblclick', (_) => {
		const v = DEFAULT_VALUES[`gpio_${x}`];
		el.value = v;
		$(`#image_${x}-show`).textContent = v;
		saveValue('gpio', x);
	});
...
loadConfig().then(() => { $('#preview').src = '/x/ch0.mjpg' });
 	if ($(`#image_alt_sensor`).checked) {
		loadConfig().then(() => { $('#preview1').src = '/x/ch2.mjpg' });
}
</script>
