...
static ssize_t sensor_i2c_bus_read(struct file *file, char __user *buf, size_t count, loff_t *ppos);
...
void sensor_common_init(struct sensor_info *info) {
	...
	proc_create("jz/sensor/i2c_bus", 0444, NULL, &i2c_bus_fops);
...
void sensor_common_exit(void) {
	...
	remove_proc_entry("jz/sensor/i2c_bus", NULL);
...
void sensor1_common_init(struct sensor_info *info) {
	sensor_info_ptr = info;

	// Create the /proc/jz/sensor1 directory
	proc_mkdir("jz/sensor1", NULL);

	// Create proc entries under /proc/jz/sensor1
	proc_create("jz/sensor1/name", 0444, NULL, &name_fops);
	proc_create("jz/sensor1/chip_id", 0444, NULL, &chip_id_fops);
	proc_create("jz/sensor1/version", 0444, NULL, &version_fops);
	proc_create("jz/sensor1/min_fps", 0444, NULL, &min_fps_fops);
	proc_create("jz/sensor1/max_fps", 0444, NULL, &max_fps_fops);
	proc_create("jz/sensor1/actual_fps", 0444, NULL, &actual_fps_fops);
	proc_create("jz/sensor1/i2c_bus", 0444, NULL, &i2c_bus_fops);
	proc_create("jz/sensor1/i2c_addr", 0444, NULL, &i2c_addr_fops);
	proc_create("jz/sensor1/height", 0444, NULL, &height_fops);
	proc_create("jz/sensor1/width", 0444, NULL, &width_fops);
}

void sensor1_common_exit(void) {
	// Remove proc entries and the directory
	remove_proc_entry("jz/sensor1/name", NULL);
	remove_proc_entry("jz/sensor1/chip_id", NULL);
	remove_proc_entry("jz/sensor1/version", NULL);
	remove_proc_entry("jz/sensor1/min_fps", NULL);
	remove_proc_entry("jz/sensor1/max_fps", NULL);
	remove_proc_entry("jz/sensor1/actual_fps", NULL);
	remove_proc_entry("jz/sensor1/i2c_bus", NULL);
	remove_proc_entry("jz/sensor1/i2c_addr", NULL);
	remove_proc_entry("jz/sensor1/height", NULL);
	remove_proc_entry("jz/sensor1/width", NULL);
	remove_proc_entry("jz/sensor1", NULL);
}
