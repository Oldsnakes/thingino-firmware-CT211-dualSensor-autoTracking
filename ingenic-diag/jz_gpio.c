...
/* ### TW: add power clock check */
#define CPM_BASE		0x10000000
#define CPM_CLKGR0		0x20
#define CPM_CLKGR1		0x28
/* ### TW: add I2C reg check */
#define I2C_BASE		0x10050000
#define I2C0_START		0x0000
#define I2C1_START		0x1000
#define I2C2_START		0x2000
...
typedef struct {
	volatile const uint32_t _rsvd0[8];
	volatile uint32_t CLKGR0;
	volatile const uint32_t _rsvd1[2];
	volatile uint32_t CLKGR1;
} XHAL_CPM_HandleTypeDef;

typedef struct {
	volatile uint32_t I2C_REG0;
	volatile const uint32_t _rsvd0[2];
	volatile uint32_t CLKGR1;
} XHAL_I2C_HandleTypeDef;

static void *phys_mem = NULL;
static void *phys_mem1 = NULL;
static void *phys_mem2 = NULL;
...
uint32_t I2C_PORT_WIDTH = 0x1000; // Default value

static void show_gpios() {

	volatile XHAL_CPM_HandleTypeDef *port1 = phys_mem1;
	printf("CLKGR0\t\tCLKGR1\n");
	...
	for (int i = 0; i < 3; i++) {
		...
		volatile XHAL_I2C_HandleTypeDef *port2 = phys_mem2 + i * I2C_PORT_WIDTH;

		printf("Port %c\n", 'A' + i);
		printf("================\n");

	/*  ### TW:  add show register */
		printf("INT\t\tMASK\t\tPAT1\t\tPAT0\n");
...
int main(int argc, char **argv) {
	...
	phys_mem1 = mmap(NULL, 0x10000, PROT_READ|PROT_WRITE, MAP_SHARED, fd, CPM_BASE);
	phys_mem2 = mmap(NULL, 0x10000, PROT_READ|PROT_WRITE, MAP_SHARED, fd, I2C_BASE);
